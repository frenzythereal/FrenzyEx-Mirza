...
/** Mirza 0.5.4 routes the admin back button through $backmenu_menus. A step
 * with no entry there falls through to the admin MAIN menu, so a FrenzyEx
 * settings prompt would not return to the FrenzyEx panel the way every
 * built-in gateway does. Cosmetic, but it makes this gateway the odd one out.
 *
 * Guarded and OPTIONAL: the registry does not exist before 0.5.4 and this same
 * package still installs onto 0.3 through 0.5.3, where the anchor is absent and
 * the patch is skipped. $FrenzyExManage comes from keyboard.php, which index.php
 * requires long before admin.php, both at global scope. */
$ADMIN_BACKMENU_BLOCK = <<<'PHPBLOCK'
// >>> FRENZYEX GATEWAY — installed by install.sh, do not edit inside these markers
...
$TARGETS = [
    'index.php' => [
        'patches' => [
            ...
            [
                // ── Mirza 0.5.5+ ONLY ──────────────────────────────────────
                // 0.5.5 put the whole payment block behind an allowlist of
                // $datain values, and `frenzyex` is not in it, so the handler
                // below became unreachable: the button still rendered (it lives
                // in keyboard.php) but tapping it fell through to an error.
                //
                // Rather than rewriting their array -- which insert-before
                // cannot do reversibly -- the SAME handler is placed BEFORE the
                // gate, where $datain is still ours to claim. The block opens
                // with `} elseif` and closes on the gate's own `} elseif`, so it
                // composes exactly as it does at the zarinpal anchor.
                //
                // On 0.5.5 the handler therefore appears twice: this one (live)
                // and the one inside the gate (dead code, never reached for
                // frenzyex). Harmless -- they are separate elseif chains -- and
                // it keeps ONE block definition and a byte-exact `remove`.
                'anchor' => '} elseif ($user[\'step\'] == "get_step_payment" && in_array($datain, ["cart_to_offline", "aqayepardakht", "zarinpal", "plisio", "nowpayment", "iranpay1", "iranpay2", "iranpay4", "iranpay3", "digitaltron", "startelegrams"])) {',
                'block' => $INDEX_BLOCK,
                'expected' => 1,
                'optional' => true,
            ],
    ...
    'admin.php' => [
        'patches' => [
            ...
            [
                // 0.5.4+ only. Single-quoted so the callback's own double
                // quotes and dollar signs need no backslash escaping.
                'anchor' => '$backmenu_register(["addchannel", "getremark", "getlinkjoin", "removechannel"], $channelkeyboard);',
                'block' => $ADMIN_BACKMENU_BLOCK,
                'expected' => 1,
                'optional' => true,
            ],
        ],
    ],
    // Optional: present in 0.3.x/0.4.x, REMOVED in 0.5.x (the file was rewritten
    // as a lean PDO loop with no gateway label map). Purely cosmetic -- it names
    // the gateway in the expired-invoice report -- so its absence must not stop
    // the install.
...
foreach ($TARGETS as $file => $spec) {
    $path = "$dir/$file";
    if (!is_file($path)) {
        // An OPTIONAL target may legitimately not exist: Mirza deletes files
        // between releases. Only a required one is fatal.
    ...
    if ($cmd === 'apply') {
        foreach ($spec['patches'] as $patch) {
            ...
            // An OPTIONAL patch is skipped when the anchor is GONE (count 0):
            // the thing it decorated no longer exists, so there is nothing to
            // decorate and nothing is wrong. Mirza 0.5 rewrote
            // cronbot/payment_expire.php from 39 lines to a 20-line PDO loop and
            // deleted the gateway label map this patch targets -- that single
            // dead anchor is what made the whole installer refuse 0.5.x, even
            // though the other eight anchors match 0.3.1 through 0.5.4 exactly.
            //
            // A WRONG count (not 0, not $expected) is still fatal: that means a
            // customized or unexpected file, which is precisely what the strict
            // check exists to catch.
            if ($count === 0 && !empty($patch['optional'])) {
                continue;
            }
            if ($count !== $expected) {
