# FrenzyEx Gateway for Mirza Bot — v2.1.5

## 2.1.5 — Mirza 0.5.4 back-menu registration

Mirza 0.5.4 introduced `$backmenu_register()`, a registry deciding which keyboard
the admin back button returns to. Every built-in gateway was enrolled; FrenzyEx
was not, so backing out of a FrenzyEx settings prompt landed on the admin main
menu instead of the FrenzyEx panel. A new OPTIONAL patch registers the six
`frenzyex_set_*` steps. It is skipped silently on 0.3 through 0.5.3, where the
registry does not exist.

0.5.4 is now verified: all eight required anchors match, the patcher applies, the
three patched files lint, the admin keyboard decodes with FrenzyEx as its own row,
and removal restores the originals byte for byte.
...
Supported: **0.3**, **0.3.1**, **0.4**, **0.4.1**, **0.5**, **0.5.2**, **0.5.3** and **0.5.4**
(any `0.5.x` is accepted).

> Mirza **0.5** rewrote `cronbot/payment_expire.php` and removed the gateway
> label map. That one cosmetic patch is now optional, so its absence no longer
> stops the install. The other eight patch anchors are byte-identical from
> 0.3.1 through 0.5.4 and are still checked strictly.
