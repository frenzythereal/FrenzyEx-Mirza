...
VERSION="2.1.5"
SUPPORTED="0.3 0.3.1 0.4 0.4.1 0.5 0.5.1 0.5.2 0.5.3 0.5.4"
# Any future 0.5.x is accepted too: all nine patch anchors are byte-identical
# from 0.3.1 through 0.5.4, and patcher.php still refuses per-anchor if a
# release ever moves one. The glob widens the prompt, not the safety check.
SUPPORTED_GLOBS="0.5.*"

# ── PHP binary ────────────────────────────────────────────────────────────────
# settings.php loads Mirza's config.php, which constructs a PDO using
# PDO::MYSQL_ATTR_INIT_COMMAND. A php CLI without pdo_mysql fatals there. Some
# servers ship a default `php` that lacks it while a versioned binary has it,
# so probe for one that actually works instead of trusting `php`.
PHP=""
pick_php() {
  local c
  for c in php php8.4 php8.3 php8.2 php8.1 php8.0 php7.4; do
    command -v "$c" >/dev/null 2>&1 || continue
    "$c" -r 'exit(extension_loaded("pdo_mysql") ? 0 : 1);' >/dev/null 2>&1 || continue
    PHP="$c"; return 0
  done
  # Last resort: any php* on PATH with pdo_mysql.
  for c in $(compgen -c 2>/dev/null | grep -E '^php[0-9.]*$' | sort -u); do
    command -v "$c" >/dev/null 2>&1 || continue
    "$c" -r 'exit(extension_loaded("pdo_mysql") ? 0 : 1);' >/dev/null 2>&1 || continue
    PHP="$c"; return 0
  done
...
install_deps() {
  ...
  pick_php || { err "هیچ نسخه‌ای از php با افزونهٔ pdo_mysql پیدا نشد. (مثلاً: apt install php8.2-mysql)"; exit 1; }
  info "PHP مورد استفاده: $PHP"
}
...
version_supported() {
  ...
  for g in $SUPPORTED_GLOBS; do [[ "$MIRZA_VER" == $g ]] && return 0; done
  return 1
}
...
configure_keys() {
  ...
  if [[ "$(printf '%s' "${on:-y}" | tr '[:upper:]' '[:lower:]')" == "n" ]]; then
    "$PHP" "$WORK/settings.php" "$MIRZA_DIR" set statusfrenzyex 0
    info "درگاه خاموش ماند."
  else
    "$PHP" "$WORK/settings.php" "$MIRZA_DIR" set statusfrenzyex 1
    ok "درگاه روشن شد."
  fi
}
...
uninstall_gateway() {
  ...
  "$PHP" "$WORK/settings.php" "$MIRZA_DIR" set statusfrenzyex 0 >/dev/null 2>&1
